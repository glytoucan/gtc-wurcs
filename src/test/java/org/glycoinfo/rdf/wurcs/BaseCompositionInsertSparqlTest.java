...
@SpringApplicationConfiguration(classes = {BaseCompositionInsertSparqlTest.class, VirtSesameTransactionConfig.class })
@Configuration
@EnableAutoConfiguration
public class BaseCompositionInsertSparqlTest {

	public static Logger logger = (Logger) LoggerFactory
			.getLogger(BaseCompositionInsertSparqlTest.class);
	
	@Autowired
	SparqlDAO sparqlDAO;

	@Bean
	 BaseCompositionInsertSparql getBaseCompositionInsertSparql() {
		BaseCompositionInsertSparql ins = new BaseCompositionInsertSparql();
	...
	@Bean
	GRABSequenceSelectSparqlSubsumedBy getBaseCompositionSelectSparqlSubsumedBy() {
...
//				getGlycosidic_topologyInsertSparql().getSparql());
	...
	public void insertSparql() throws SparqlException {
		...
		List<SparqlEntity> list = sparqlDAO.query(getBaseCompositionSelectSparql());
		for (SparqlEntity sparqlEntity : list) {
			String output = sparqlEntity.getValue("s");
			//Assert.assertEquals("http://purl.jp/bio/12/glyco/glycan#saccharide", output);
		}
	...
	public void insertSelectSparql_topology_by() throws SparqlException {
		...
		List<SparqlEntity> list = sparqlDAO.query(getBaseCompositionSelectSparqlSubsumedBy());
		if (list.iterator().hasNext()) {
			SparqlEntity se = list.iterator().next();
	...
	public void selectSparql() throws SparqlException {
		GRABSequenceSelectSparqlSubsumes sss = getBaseCompositionSelectSparql();
